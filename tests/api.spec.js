...
// })
test('test 7: extract an API token ', async ({request}) => {
    console.log('1. send POST request to login and get the token...');
    ...
    console.log('2. verify status code 200');
    expect(response.status()).toBe(200);
    console.log('3. unpack the JSON response body');
    const responseBody = await response.json();
    console.log('4. extract the token from the response body');
    const myToken = responseBody.token;
    expect(myToken).toBeDefined();
    console.log('5. print the token value');
    console.log('myToken: ', responseBody.token);
        const deleteResponse = await request.delete('https://restful-booker.herokuapp.com/booking/1', {
        headers: {
